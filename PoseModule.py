...
class posture_detector():
    def __init__(self, mode=False, up_body=False, smooth=True,
                 detection_con=False, track_con=False):
        self.mode = mode
...
def main():
    cap = cv2.VideoCapture('hammer curl.mp4')
    ...
    while True:
        ...
        if len(landmark_list) != 0:
            # right_leg_angle = detector.find_angle(img, 12, 14, 16)
        ...
        # if len(landmark_list) != 0:
            ...
            cv2.circle(
            img, (landmark_list[14][1], landmark_list[14][2]), 15, (151, 151, 0), cv2.FILLED)

            cv2.circle(
            img, (landmark_list[13][1], landmark_list[13][2]), 15, (51, 153, 102), cv2.FILLED)

            cv2.circle(
            img, (landmark_list[23][1], landmark_list[23][2]), 15, (153, 204, 0), cv2.FILLED)

            cv2.circle(
            img, (landmark_list[26][1], landmark_list[26][2]), 15, (153, 204, 204), cv2.FILLED)
            cv2.circle(
            img, (landmark_list[25][1], landmark_list[25][2]), 15, (255, 255, 255), cv2.FILLED)
            cv2.circle(
            img, (landmark_list[12][1], landmark_list[12][2]), 15, (204,204,225), cv2.FILLED)
            cv2.circle(
                img, (landmark_list[11][1], landmark_list[11][2]), 15, (0, 225, 225), cv2.FILLED)
            cv2.circle(
                img, (landmark_list[24][1], landmark_list[24][2]), 15, (225, 0, 0), cv2.FILLED)
